...
#     print(len(numbers), set(numbers))
#     print(len(currency), set(currency))
#     print(len(percentages), set(percentages))
#     print(len(weird_i), set(weird_i))
#     print(len(weird_i2), set(weird_i2))
#     print(len(rest), set(rest))
...
def plot_alignment_examples(labels, base = c.data_base / 'examples') :
    ...
    for title in set( [ f.stem[:10] for f in all_files ] ) :
        ...
        total_end = max( transcript[-1]['end'] for transcript in transcripts if transcript )

        extra_transcript = []
        # create gaps
        for pre, post in zip([{'end' : 0}] + transcripts[1], transcripts[1] + [{'start' : total_end}]) :
            start_a = pre['end']
            end_a = post['start']
            gap = end_a - start_a
            if gap > 0.2 :
                start, end = calculate_gap(manual, start_a, end_a, total_end)
                if end - start > 0.1 :
                    extra_transcript.append({
                        'word' : ' '.join( w['word'] for w in manual if start <= w['start'] and w['end'] <= end),
                        'original' : False,
                        'start' : start,
                        'end' : end
                    })
        a, b = alignment.align([ w['word'] for w in manual ], [ w['word'] for w in transcripts[1] ])
        alignment.print_words(a, b)
        print()
        visual.plot_word_positions(transcripts + [extra_transcript], title, labels + ['gaps'])

# data.collect_alignment_examples(c.manual_seg_dir, [(c.automatic_align_dir / '0', None)], c.audio_dir, 5, save_dir)
# plot_alignment_examples(['manual', 'whisper v3'], save_dir)
        
import tasks.hesitation_dataset as dataset

# 1 hour of gaps is 4067 files
# dataset.create_dataset_files(1)
