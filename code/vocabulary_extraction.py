...
def get_vocabulary_set(source_directory, destination_directory) :
    english_most_commen = utils.read_file("code\\sample_data\\vocabulary\\english_most_commen_5000.txt").split('\n')
    superset = set("uhmoyaein-")
    common_limit = 100
    
    vocabulary = get_vocabulary(source_directory)
    write_vocabulary(destination_directory + "\\vocabulary.txt", vocabulary)
    common = {w: c for w, c in vocabulary.items() if c >= common_limit}
    write_vocabulary(destination_directory + "\\vocabulary_common.txt", common)
    ...
    write_vocabulary(destination_directory + "\\hesitations.txt", hesitations)
    stutter = {w: c for w, c in vocabulary.items() if w[-1] == '-' or w[0] == '-'}
    write_vocabulary(destination_directory + "\\stutter.txt", stutter)
